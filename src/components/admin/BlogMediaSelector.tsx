...
interface BlogMediaSelectorProps {
...
export const BlogMediaSelector = ({
  mediaUrl,
  mediaType,
  onMediaChange,
  label = "מדיה ראשית",
  className
}: BlogMediaSelectorProps) => {
    ...
    <div className={cn("w-full p-6 bg-white border border-gray-100 rounded-xl shadow-sm", className)}>
      ...
      {mode === "upload" && (
        ...
        >
          ...
          <p className="text-xs text-gray-400 mt-1">MP4, WebM, PNG, JPG (עד 50MB)</p>
        </div>
      )}
...
export default BlogMediaSelector;
